...
import {
  ...
  FileText, Gift, Handshake, LayoutDashboard, Loader2, LogOut, Mail, Menu, Search, Settings,
...
const AdminFreshersCampaignPage = lazy(() => import("@/pages/admin/AdminFreshersCampaignPage"));
...
const navItems = [
  ...
  { to: "/admin/freshers-campaign", label: "KIET Freshers", icon: Gift },
...
function UsersPage({ openUser }) {
  ...
  return <><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs uppercase tracking-[0.25em] text-indigo-300">Directory</p><h1 className="font-display mt-2 text-3xl font-extrabold">Users</h1><p className="mt-2 text-sm text-zinc-400">Search and inspect registered students.</p></div><div className="relative w-full lg:w-80"><Search size={16} className="absolute left-3 top-3 text-zinc-500" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, email, college" className="w-full rounded-xl border border-white/10 bg-white/[0.04] py-2.5 pl-9 pr-3 text-sm outline-none focus:border-indigo-400" /></div></div>
    ...
    <div className="mt-5 overflow-x-auto rounded-2xl border border-white/10"><table className="w-full min-w-[920px] text-left text-sm"><thead className="border-b border-white/10 bg-white/[0.03] text-xs uppercase tracking-wider text-zinc-500"><tr><th className="p-4">Name</th><th className="p-4">Email</th><th className="p-4">College</th><th className="p-4">Savvy Points</th><th className="p-4">Status</th><th className="p-4">Joined</th><th className="p-4">Actions</th></tr></thead><tbody>{users.isLoading ? <tr><td colSpan="7" className="p-10 text-center text-zinc-500"><Loader2 className="mx-auto animate-spin" size={22} /></td></tr> : users.data?.items.length ? users.data.items.map((user) => <tr key={user.id} className="border-b border-white/[0.06] last:border-0"><td className="p-4 font-semibold">{user.name || "—"}</td><td className="p-4 text-zinc-400">{user.email}</td><td className="p-4 text-zinc-400">{user.college || "—"}</td><td className="p-4"><p className="font-display text-base font-extrabold text-amber-200">{(user.savvy_points_balance || 0).toLocaleString("en-IN")}</p><p className="text-[10px] text-zinc-500">{(user.savvy_points_lifetime || 0).toLocaleString("en-IN")} lifetime · {user.savvy_tier}</p></td><td className="p-4"><StatusBadge status={user.verification_status} /></td><td className="p-4 text-zinc-400">{dateText(user.created_at)}</td><td className="p-4"><button onClick={() => openUser(user.id)} className="rounded-lg bg-white/8 px-3 py-1.5 text-xs font-semibold hover:bg-white/15">View</button></td></tr>) : <tr><td colSpan="7" className="p-10 text-center text-zinc-500">No users match this filter.</td></tr>}</tbody></table></div>
...
function UserDialog({ userId, onClose, selectedVerification, onReview }) {
  ...
  return <><Dialog open={Boolean(userId)} onOpenChange={(open) => !open && onClose()}><DialogContent className="max-h-[88vh] max-w-4xl overflow-y-auto bg-[#111114] text-white"><DialogHeader><DialogTitle>{detail.isLoading ? "Loading profile…" : user?.name || "Student profile"}</DialogTitle><DialogDescription className="text-zinc-400">Review identity details, Savvy progress, and uploaded documents.</DialogDescription></DialogHeader>{detail.isError ? <p className="text-rose-300">This user is no longer available.</p> : detail.isLoading ? <div className="h-64 grid place-items-center"><Loader2 className="animate-spin" /></div> : <div className="space-y-7"><div className="grid gap-4 rounded-2xl bg-white/[0.04] p-4 sm:grid-cols-2 lg:grid-cols-3"><div><p className="text-xs text-zinc-500">Email</p><p className="mt-1 text-sm">{user?.email}</p></div><div><p className="text-xs text-zinc-500">College</p><p className="mt-1 text-sm">{user?.college || "—"}</p></div><div><p className="text-xs text-zinc-500">Verification status</p><div className="mt-1"><StatusBadge status={user?.verification_status} /></div></div><div><p className="text-xs text-zinc-500">Course / academic year</p><p className="mt-1 text-sm">{user?.course || "—"} {user?.year ? `• ${user.year}` : ""}</p></div><div><p className="text-xs text-zinc-500">Student ID</p><p className="mt-1 text-sm">{verification?.student_id_number || "—"}</p></div><div><p className="text-xs text-zinc-500">Submitted</p><p className="mt-1 text-sm">{dateText(verification?.submitted_at)}</p></div><div className="rounded-xl border border-amber-300/10 bg-amber-300/[0.05] p-3"><p className="flex items-center gap-1.5 text-xs text-amber-200"><Trophy size={12} /> Savvy Points balance</p><p className="mt-1 font-display text-2xl font-extrabold text-white">{(user?.savvy_points_balance || 0).toLocaleString("en-IN")}</p></div><div className="rounded-xl border border-violet-300/10 bg-violet-400/[0.05] p-3"><p className="text-xs text-violet-200">Lifetime earned</p><p className="mt-1 font-display text-2xl font-extrabold text-white">{(user?.savvy_points_lifetime || 0).toLocaleString("en-IN")}</p></div><div className="rounded-xl border border-white/10 bg-white/[0.03] p-3"><p className="text-xs text-zinc-500">Current tier</p><p className="mt-1 font-display text-lg font-extrabold text-white">{user?.savvy_tier}</p></div></div><div><h3 className="font-display text-lg font-bold">Uploaded documents</h3><div className="mt-3 grid gap-4 sm:grid-cols-2"><ImagePreview label="College ID card" src={verification?.college_id_image} onOpen={(src, label) => setImage({ src, label })} /><ImagePreview label="Selfie with college ID" src={verification?.selfie_image || verification?.selfie_with_id} onOpen={(src, label) => setImage({ src, label })} /></div></div>{user?.verification_rejection_reason && <p className="rounded-xl bg-rose-500/10 p-3 text-sm text-rose-200">Previous rejection: {user.verification_rejection_reason}</p>}{verification?.status === "pending" && <DialogFooter><button onClick={() => onReview(verification, "approve")} className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-bold text-black">Approve</button><button onClick={() => onReview(verification, "reject")} className="rounded-xl bg-rose-500 px-4 py-2 text-sm font-bold text-white">Reject</button></DialogFooter>}</div>}</DialogContent></Dialog>
...
export default function AdminPortal() {
  ...
  else if (pathname.startsWith("/admin/freshers-campaign")) page = <Suspense fallback={<div className="min-h-[420px] grid place-items-center"><Loader2 className="animate-spin text-indigo-300" /></div>}><AdminFreshersCampaignPage /></Suspense>;
