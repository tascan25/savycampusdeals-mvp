...
export function OutletsExplorer() {
  ...
  const citiesQuery = useQuery({
    queryKey: queryKeys.outlets.cities(),
    queryFn: apiListOutletCities,
  });
  ...
  const outletsQuery = useQuery({
    queryKey: queryKeys.outlets.list(filters),
    queryFn: () => apiListOutlets(filters),
  });
  ...
  const cuisines = useMemo(
    () => [
      "all",
      ...Array.from(
        new Set((outletsQuery.data ?? []).map((outlet) => outlet.cuisine).filter(Boolean)),
      ).sort(),
    ],
    [outletsQuery.data],
  );
  ...
  const areaLabel =
    city !== "all"
      ? city
      : visibleOutlets[0]?.city || (coords ? "Current location" : "Nearby campus");
  ...
  return (
    <FlatList
      ...
      refreshing={outletsQuery.isRefetching}
      onRefresh={() => void outletsQuery.refetch()}
      renderItem={({ item, index }) => (
        <View style={styles.outletRow}>
          <NearbyOutletRow outlet={item} onPress={() => router.push(`/outlet/${item.id}`)} />
      ...
      ListHeaderComponent={
        <>
          <Pressable
            ...
            accessibilityLabel={`Area: ${areaLabel}. Tap to change`}
          >
            <Ionicons name="location" size={16} color={color.textPrimary} />
            <AppText variant="small" numberOfLines={1} style={styles.locationLabel}>
              {areaLabel}
            </AppText>
          ...
          <FlatList
            horizontal
            data={["open", "nearby", "cuisine"] as const}
            ...
            renderItem={({ item }) =>
              item === "open" ? (
                <Pressable
                  ...
                  style={[styles.filter, openOnly && styles.filterActive]}
                >
                  <View style={[styles.statusDot, !openOnly && styles.statusDotMuted]} />
                  <AppText variant="small" color={openOnly ? "#DDD6FE" : color.textSecondary}>
                    Open now
                  </AppText>
                </Pressable>
              ) : item === "nearby" ? (
                <Pressable
                  ...
                  style={[styles.filter, nearbyOnly && styles.filterActive]}
                >
                  <Ionicons
                    name={locationState === "loading" ? "hourglass-outline" : "navigate-outline"}
                    size={14}
                    color={nearbyOnly ? "#C4B5FD" : color.textSecondary}
                  />
                  <AppText variant="small" color={nearbyOnly ? "#DDD6FE" : color.textSecondary}>
                    {locationState === "loading" ? "Locating…" : `Within ${radiusKm} km`}
                  </AppText>
                  {nearbyOnly ? (
                    <Pressable
                      onPress={(event) => {
                        event.stopPropagation();
                        setRadiusKm((value) => (value === 2 ? 5 : 2));
                      }}
                      hitSlop={8}
                    >
                ...
                <Pressable
                  onPress={cycleCuisine}
                  style={[styles.filter, cuisine !== "all" && styles.filterActive]}
                >
                  <AppText
                    variant="small"
                    color={cuisine !== "all" ? "#DDD6FE" : color.textSecondary}
                  >
                    {cuisine === "all" ? "All categories" : cuisine}
                  </AppText>
                  <Ionicons
                    name="chevron-down"
                    size={14}
                    color={cuisine !== "all" ? "#C4B5FD" : color.textSecondary}
                  />
                </Pressable>
              )
            }
          ...
          {locationState === "denied" ? (
            <AppText variant="caption" color={color.amber} style={styles.locationMessage}>
              Location access was unavailable. Choose an area above or enable location in Settings.
            </AppText>
          ) : null}
          ...
          <NearbyMapPreview
            ...
            areaLabel={areaLabel}
            locating={locationState === "loading"}
            onLocate={() => void requestNearby()}
            onPressOutlet={(outlet) => router.push(`/outlet/${outlet.id}`)}
          />
            ...
            <View style={styles.sheetHeader}>
              <View>
                ...
                <AppText variant="caption" color={color.textTertiary}>
                  {visibleOutlets.length} places found
                </AppText>
              </View>
              {nearbyOnly ? (
        ...
        outletsQuery.isLoading ? (
          ...
          <View style={styles.empty}>
            ...
            <AppText variant="small" color={color.textTertiary} style={styles.emptyCopy}>
              Try another area or clear one of the filters.
            </AppText>
          </View>
        )
      }
      ListFooterComponent={visibleOutlets.length ? <View style={styles.sheetFooter} /> : null}
    />
  );
}
