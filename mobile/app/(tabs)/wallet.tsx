...
export default function WalletTab() {
  ...
  const groups = useMemo(() => {
    const result: Record<WalletStatus, CouponClaimResult[]> = {
      active: [],
      redeemed: [],
      expired: [],
    };
  ...
  return (
    <Screen edges={["top"]}>
      <FlatList
        ...
        refreshing={couponsQuery.isRefetching}
        onRefresh={() => void couponsQuery.refetch()}
        renderItem={({ item, index }) =>
          index === 0 ? (
            <View style={styles.coupons}>
              <CouponTicket coupon={item} onPress={() => setSelected(item)} />
            </View>
          ) : (
            <View style={[styles.compactCoupon, index === 1 && styles.firstCompactCoupon]}>
              <CouponCompactRow coupon={item} onPress={() => setSelected(item)} />
            </View>
          )
        }
        ListHeaderComponent={
            ...
            <View style={styles.header}>
              ...
              <View style={styles.headerActions}>
                <Pressable
                  ...
                  accessibilityLabel="Open Savvy Points"
                >
                  <Ionicons name="sparkles" size={14} color="#C7D2FE" />
                  <AppText variant="small">
                    {(user?.savvy_points_balance ?? 0).toLocaleString("en-IN")}
                  </AppText>
                </Pressable>
                <Pressable
          ...
          couponsQuery.isLoading ? (
            ...
            <View style={styles.empty}>
              <View style={styles.emptyIcon}>
                <Ionicons name="ticket-outline" size={28} color="#A5B4FC" />
              </View>
              ...
              <AppText variant="small" color={color.textTertiary} style={styles.emptyCopy}>
                {status === "active"
                  ? "Claim a student deal and it will appear here, ready at the counter."
                  : "Your coupon history will stay organised here."}
              </AppText>
              {status === "active" ? (
                <Pressable
        ...
        ListFooterComponent={
          <Pressable
            ...
            onPress={() => router.push("/brand-claims" as never)}
            accessibilityRole="button"
          >
            <View style={styles.brandLinkIcon}>
              <Ionicons name="open-outline" size={18} color="#C7D2FE" />
            </View>
            <View style={styles.brandLinkCopy}>
              ...
              <AppText variant="caption" color={color.textTertiary}>
                Reopen deals fulfilled on brand websites
              </AppText>
            </View>
      ...
      <CouponDetailModal
        ...
        onClose={() => {
          setSelected(null);
          void queryClient.invalidateQueries({ queryKey: queryKeys.coupons.mine() });
        }}
      />
    </Screen>
  );
